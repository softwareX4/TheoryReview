...
##### DNS:因特网的目录服务
- 主机的标识方法：**主机名**（易记）、**IP地址**（定长）
  - IP地址：**4个字节**组成，**层次结构**
  自左向右地址逐渐具体
- 域名服务（DNS,Domain Name System）
主机名到IP地址转换的目录服务
  - 由**分层DNS服务器**实现的**分布式数据库**
  - 使主机能够查询分布式数据库的**应用层协议**
- DNS服务器通常是运行**BIND**（Berkeley Internet Name Domain）软件的UNIX机器。DNS协议运行在**UDP**之上，使用**53端口**。
- 用户浏览器请求URL
  - 用户主机运行DNS应用客户端
  - 浏览器从URL中抽取主机名，并传给DNS客户端
  - DNS客户向DNS服务器发送一个包含主机名的请求
  - DNS收到回答报文，包含该主机名的IP地址
  - 浏览器向位于该IP地址的80端口的HTTP服务进程发起一个**TCP连接**
- DNS重要服务
  - 主机别名（host aliasing）
  - 邮件服务器别名（mail server aliasing）
  - 负载分配（load distribution）
  DNS也用于在冗余服务器之间进行负载分配，一个IP地址集合与同一个规范主机名相联系，客户对映射到某地址集合的名字发出DNS请求，该服务器用IP地址的整个集合进行响应。
...
- DNS缓存
在一个请求链中，当某DNS服务器接收到一个DNS回答，它能将该回答中的信息缓存在本地存储器中。如主机名/IP地址对。当另一个相同主机名的查询到达时，该服务器能够提供IP地址，即使它不是权威服务器。
- DNS记录和报文
资源记录是一个4元组：（Name,Value,Type,TTL）

Type|Name|Value|含义|EG
|:-:|:-:|:-:|:-:|:-:|
A|主机名|IP地址|标准主机名到IP地址的映射|（relayl.bar.foo.com,145.37.93.126,A）
NS|域|权威DNS服务器的主机名（知道如何获得该域中主机IP地址）|沿着查询链来路由DNS查询|（foo.com,dns.foo.com,NS）
CNAME||别名为Name的主机对应的规范主机名|向查询的主机提供一个主机名对应的规范主机名|（foo.com,relayl.bar.foo.com,CNAME）
MX||别名为Name的邮件服务器的规范主机名|允许邮件服务器主机名具有简单的别名||
